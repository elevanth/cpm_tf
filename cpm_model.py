...
class CPM_Model(object):
    ...
    def build_model(self, input_image, batch_size):
        ...
        with slim.arg_scope([slim.conv2d],
            ...
            with tf.variable_scope('sub_stages'):
                net = slim.conv2d(input_image, 64, scope='sub_conv1')
                net = slim.conv2d(net, 64, scope='sub_conv2')
                net = slim.max_pool2d(net, [2, 2], padding='SAME', scope='sub_pool1')
                net = slim.conv2d(net, 128, scope='sub_conv3')
                net = slim.conv2d(net, 128, scope='sub_conv4')
                net = slim.max_pool2d(net, [2, 2], padding='SAME', scope='sub_pool2')
                net = slim.conv2d(net, 256, scope='sub_conv5')
                net = slim.conv2d(net, 256, scope='sub_conv6')
                net = slim.conv2d(net, 256, scope='sub_conv7')
                net = slim.conv2d(net, 256, scope='sub_conv8')
                net = slim.max_pool2d(net, [2, 2], padding='SAME', scope='sub_pool3')
                net = slim.conv2d(net, 512, scope='sub_conv9')
                net = slim.conv2d(net, 512, scope='sub_conv10')
                net = slim.conv2d(net, 256, scope='sub_conv11')
                net = slim.conv2d(net, 256, scope='sub_conv12')
                net = slim.conv2d(net, 256, scope='sub_conv13')
                net = slim.conv2d(net, 256, scope='sub_conv14')

                self.sub_stage_img_feature = slim.conv2d(net, 128,
                                                         scope='sub_stage_img_feature')

            with tf.variable_scope('stage_1'):
                conv1 = slim.conv2d(self.sub_stage_img_feature, 512, kernel_size=[1, 1], scope='conv1')
                self.stage_heatmaps.append(slim.conv2d(conv1, self.joints, kernel_size=[1, 1],
                                                       scope='stage_heatmap'))
    ...
    def _middle_conv(self, stage):
        with tf.variable_scope('stage_' + str(stage)):
            self.input_featuremap = tf.concat([self.stage_heatmaps[stage - 2],
            ...
            with slim.arg_scope([slim.conv2d],
                                ...
                                activation_fn=tf.nn.relu,
                                weights_initializer=tf.contrib.layers.xavier_initializer()):
                mid_net = slim.conv2d(self.input_featuremap, 128, scope='mid_conv1')
                mid_net = slim.conv2d(mid_net, 128, scope='mid_conv2')
                mid_net = slim.conv2d(mid_net, 128, scope='mid_conv3')
                mid_net = slim.conv2d(mid_net, 128, scope='mid_conv4')
                mid_net = slim.conv2d(mid_net, 128, scope='mid_conv5')
                mid_net = slim.conv2d(mid_net, 128, kernel_size=[1, 1], scope='mid_conv6')

                self.output_heatmap = slim.conv2d(mid_net, self.joints, kernel_size=[1, 1],
                                                  scope='mid_conv7')
                self.stage_heatmaps.append(self.output_heatmap)

    def build_loss(self, heatmaps, lr, lr_deacy_rate, lr_decay_step):
        self.labels = heatmaps
        self.total_loss = 0
        self.learning_rate = lr
        self.lr_decay_rate = lr_deacy_rate
        self.lr_decay_step = lr_decay_step

        for stage in range(self.stages):
            with tf.variable_scope('stage'+str(stage+1)+'_loss'):
                self.stage_loss[stage] = tf.nn.l2_loss(self.stage_heatmaps[stage] - self.labels,
                                                       name='l2_loss') / self.batch_size
            tf.summary.scalar('stage'+str(stage+1)+'_loss', self.stage_loss[stage])

        with tf.variable_scope('total_loss'):
            self.total_loss = sum(self.stage_loss)
